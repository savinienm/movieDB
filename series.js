...
Button.addEventListener("click", function () {
  ...
  fetch(
    "https://api.themoviedb.org/3/search/tv?api_key=153bac550dbfc9bda1b2f5ef2f99a808&language=fr&query=" +
      input.value
  )
    .then((resp) => resp.json())
    .then((data) => {
  ...
  function Movie(data) {
    for (let i in data.results) {
      ...
      let date = data.results[i].first_air_date.substring(0, 4);
...
async function main() {
  const popularMovie = await fetch(
    `https://api.themoviedb.org/3/tv/popular?api_key=153bac550dbfc9bda1b2f5ef2f99a808&language=fr&page=1`
  )
    .then((resultat) => resultat.json())
    .then((json) => json);

  const topRatedMovie = await fetch(
    `https://api.themoviedb.org/3/tv/top_rated?api_key=153bac550dbfc9bda1b2f5ef2f99a808&language=fr&page=1`
  )
    .then((resultat) => resultat.json())
    .then((json) => json);

  displayPopularMovieInfos(popularMovie);
  displayTopRatedMovieInfos(topRatedMovie);
  displayHeroInfos(popularMovie);
}

function displayPopularMovieInfos(data) {
  for (let i in data.results) {
...
function displayTopRatedMovieInfos(data) {
  for (let i in data.results) {
    if (i < 4) {
...
function displayHeroInfos(data) {
  ...
  let date = data.results[0].first_air_date.substring(0, 4);
